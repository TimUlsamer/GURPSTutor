...
def convert_keywords(text: str) -> str:
    pattern = re.compile(r"\[([^\]|]+)\|(\d+)\]|\[([^\]]+)\]\((\d+)\)")

    def repl(match: re.Match) -> str:
        word = match.group(1) or match.group(3)
        page = match.group(2) or match.group(4)
        return f'<span class="pdf" data-page="{page}">{word}</span>'
...
def render_html(data, pdf_b64, pdf_name):
    ...
    for sec in data.get("sections", []):
        ...
        read_text = convert_keywords(sec.get("readaloud", ""))
        read_html = f'<div class="readaloud"><p>{read_text}</p></div>' if read_text else ""
        body_html = convert_keywords(sec.get("body", ""))
        right_html += (
...
if mode == "Browse":
    ...
    if "sections" not in st.session_state or st.session_state.get("file") != choice:
        ...
        if not st.session_state["sections"]:
            st.session_state["sections"] = [{"id": "", "title": "", "tags": [], "readaloud": "", "body": ""}]
    st.session_state["file"] = choice

    st.markdown("Use `[keyword](12)` to add PDF links in text.")

    for idx, sec in enumerate(st.session_state["sections"]):
        label = f"{sec.get('id','')}. {sec.get('title','')}" or f"Section {idx+1}"
        with st.expander(label, expanded=True):
            ...
            tags_input = st.text_input("Tags (comma-separated)", value=tags_str, key=f"tags_{idx}")
            sec["tags"] = [t.strip() for t in tags_input.split(",") if t.strip()]
            sec["readaloud"] = st.text_area("Read Aloud", value=sec.get("readaloud", ""), key=f"read_{idx}")
            sec["body"] = st.text_area("Body", value=sec.get("body", ""), key=f"body_{idx}")
            kw_col1, kw_col2, kw_col3 = st.columns([2, 1, 1])
            kw = kw_col1.text_input("Keyword", key=f"kw_{idx}")
            page = kw_col2.number_input("Page", min_value=1, step=1, key=f"pg_{idx}")
            if kw_col3.button("Insert", key=f"ins_{idx}") and kw:
                body_key = f"body_{idx}"
                st.session_state[body_key] = st.session_state.get(body_key, "") + f" [{kw}]({int(page)})"
    ...
    if st.button("Add Section"):
        st.session_state["sections"].append({"id": "", "title": "", "tags": [], "readaloud": "", "body": ""})
        st.experimental_rerun()

    def clean_sections(sections):
        cleaned = []
        for sec in sections:
            if not any(str(sec.get(f, "")).strip() for f in ["id", "title", "tags", "readaloud", "body"]):
                continue
            cleaned.append({k: sec.get(k, "") for k in ["id", "title", "tags", "readaloud", "body"]})
        return cleaned

    sections_out = clean_sections(st.session_state["sections"])

    if st.button("Save Adventure"):
        save_data = {"title": title, "sections": sections_out}
        (ADVENTURE_DIR / f"{filename}.json").write_text(json.dumps(save_data, indent=2))
        st.success("Adventure saved.")
    if st.button("Preview"):
        preview_data = {"title": title, "sections": sections_out}
