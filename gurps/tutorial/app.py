...
if mode == "Browse":
    ...
    if "sections" not in st.session_state or st.session_state.get("file") != choice:
        rows = [{**sec, "tags": ", ".join(sec.get("tags", []))} for sec in data.get("sections", [])]
        st.session_state["sections"] = pd.DataFrame(rows, columns=columns)
    ...
    def convert_rows(df):
        ...
        for row in df.to_dict("records"):
            ...
            tags = [t.strip() for t in clean_row.get("tags", "").split(",") if t.strip()]
            clean_row["tags"] = tags
