...
if mode == "Browse":
    ...
    st.markdown("""
Use `[#](pX)` syntax inside *Read Aloud* or *Body* to create a keyword that jumps to a PDF page.
Example: `[Sword](p12)`
""")

    for i, sec in enumerate(st.session_state["sections"]):
        with st.expander(f"Section {i+1}: {sec.get('title') or 'New'}", expanded=True):
            sec_id = st.text_input("ID", value=sec.get("id", ""), key=f"sec-{i}-id")
            sec_title = st.text_input("Title", value=sec.get("title", ""), key=f"sec-{i}-title")
            sec_tags = st.text_input(
                "Tags (comma-separated)", value=sec.get("tags", ""), key=f"sec-{i}-tags"
            )
            sec_read = st.text_area("Read Aloud", value=sec.get("readaloud", ""), key=f"sec-{i}-read")
            sec_body = st.text_area("Body", value=sec.get("body", ""), key=f"sec-{i}-body")
    ...
    def convert_sections(sections):
        ...
        for sec in sections:
            if not any(str(v).strip() for v in sec.values()):
                continue
            tags = [t.strip() for t in sec.get("tags", "").split(",") if t.strip()]
            read = pattern.sub(lambda m: f'<span class="pdf" data-page="{m.group(2)}">{m.group(1)}</span>', sec.get("readaloud", ""))
            body = pattern.sub(lambda m: f'<span class="pdf" data-page="{m.group(2)}">{m.group(1)}</span>', sec.get("body", ""))
            out.append({
                "id": sec.get("id", ""),
                "title": sec.get("title", ""),
                "tags": tags,
    ...
    if st.button("Save Adventure"):
        save_data = {"title": title, "sections": convert_sections(st.session_state["sections"])}
        (ADVENTURE_DIR / f"{filename}.json").write_text(json.dumps(save_data, indent=2))
        st.success("Adventure saved.")
    if st.button("Preview"):
        preview_data = {"title": title, "sections": convert_sections(st.session_state["sections"])}
